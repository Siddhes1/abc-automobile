...
function UpdateVehicals() {
  const { id } = useParams();
  const navigate = useNavigate();
  const [vehicels, setVehicals] = useState(null);
  ...
  const formik = useFormik({
    initialValues: {
      image: vehicels?.image || "",
      name: vehicels?.name || "",
      price: vehicels?.price || "",
      mileage: vehicels?.mileage || "",
      color: vehicels?.color || "",
      seats: vehicels?.seats || "",
      fuel: vehicels?.fuel || "",
      gear: vehicels?.gear || "",
      description: vehicels?.description || "",
    },
